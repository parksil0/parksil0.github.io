...
중간 부분에 `fooBar()`는 선언과 동시에 `foo()`가 할당이 된 것을 볼 수 있습니다. 그 다음줄 출력문을 보면 `fooBar()`는 함수라는 것을 알 수 있습니다. 함수를 실행하면 위의 코드 중 1번이 실행되고 hello를 리턴합니다. 맨 마지막 줄 fooBar()를 출력했기 때문에 hi! 와 hello!가 출력됩니다.
